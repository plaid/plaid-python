...
class LinkTokenCreateRequest(ModelNormal):
    ...
    def openapi_types():
        ...
        return {
            ...
            'optional_products': ([Products],),  # noqa: E501
    ...
    def _from_openapi_data(cls, client_name, language, country_codes, user, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            required_if_supported_products ([Products]): List of Plaid product(s) you wish to use only if the institution and account(s) selected by the user support the product. Institutions that do not support these products will still be shown in Link. The products will only be extracted and billed if the user selects an institution and account type that supports them.  There should be no overlap between this array and the `products`, `additional_consented_products` or `optional_products` arrays. The `products` array must have at least one product.  For more details on using this feature, see [Required if Supported Products](https://www.plaid.com/docs/link/initializing-products/#required-if-supported-products).. [optional]  # noqa: E501
