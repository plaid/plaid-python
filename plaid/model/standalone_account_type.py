...
class StandaloneAccountType(ModelNormal):
    ...
    def _from_openapi_data(cls, depository, credit, loan, investment, other, *args, **kwargs):  # noqa: E501
        ...
        Args:
            depository (str): An account type holding cash, in which funds are deposited. Supported products for `depository` accounts are: Auth (`checking` and `savings` types only), Balance, Transactions, Identity, Payment Initiation, Assets, and Investments (`cash management` type only).
