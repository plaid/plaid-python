...
class Item(ModelNormal):
    ...
    def _from_openapi_data(cls, item_id, webhook, error, available_products, billed_products, consent_expiration_time, update_type, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            products ([Products]): A list of initialized products for the Item. In almost all cases, this will be the same as the `billed_products` field. For some products, it is possible for the product to be initialized on an Item but not yet billed (e.g. Assets, before `/asset_report/create` has been called), in which case the product may appear in `products` but not in `billed_products`. . [optional]  # noqa: E501
            consented_products ([Products]): A list of products that have gone through consent collection for the Item. Only present for those enabled in the [Data Transparency](https://plaid.com/docs/link/data-transparency-messaging-migration-guide) beta. If you are not enrolled in Data Transparency, this field is not used. . [optional]  # noqa: E501
        """
